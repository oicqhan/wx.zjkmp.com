...
class task_tigerModuleProcessor extends WeModuleProcessor
{
    ...
    public function respond()
    {
        ...
        //生成海报开始
        if ($this->message['msgtype'] == 'text' || $this->message['event'] == 'CLICK' and $this->message['event'] <> 'subscribe' and $this->message['event'] <> 'SCAN') {
            if ($poster['winfo1']) {
    ...
    public function tppostmsg($openid, $fans, $poster, $sjsum, $share)
    {//任务中模版
        global $_W;
    ...
    public function tppostmsg1($openid, $fans, $poster, $sjsum, $share)
    {//任务完在模版
        global $_W;
    ...
    private function createPoster($fans, $poster, $openid)
    {
        ...
        foreach ($data as $value) {
            ...
            if ($value['type'] == 'qr') {
                ...
                $img = saveImage($fans['avatar'], $share['id']);
                file_put_contents(IA_ROOT . "/addons/task_tiger/log.txt", "\n old:" . json_encode($img), FILE_APPEND);
                mergeImage($target, $img, array('left' => $value['left'], 'top' => $value['top'], 'width' => $value['width'], 'height' => $value['height']));
                @unlink($img);
...
//            新规则（随机读取一个）：
            $acid = pdo_fetchcolumn('select acid from ' . tablename('account') . 'ORDER BY  RAND() LIMIT 1');
...
//              生成永久带参二维码
    ...
    public function setadd($location_x, $location_y)
    {//根据坐标获取微信地理位置地区
        global $_W;
        $lat = $location_x;
        $lng = $location_y;
